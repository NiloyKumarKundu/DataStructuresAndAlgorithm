...
int knapsack(int i, int curWeight) {
    if (i > n) {
		return 0;
	}
    if (dp[i][curWeight] != -1) {
	...
	int ret1 = 0, ret2 = 0;
    if (w[i] + curWeight <= totalSize) {
		ret1 = p[i] + knapsack(i + 1, w[i] + curWeight);
	}
	ret2 = knapsack(i + 1, curWeight);
	return dp[i][curWeight] = max(ret1, ret2);
}
