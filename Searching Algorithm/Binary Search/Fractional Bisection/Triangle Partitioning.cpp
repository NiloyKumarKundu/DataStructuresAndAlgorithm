...
double findAD(double ab, double ac, double bc, double R) {
	double l = 0, r = ab;
    rep(i, 0, 100) {
		double ad = (l + r) / 2.0;
		double x = ad / ab;
		...
		double ABC = sqrt(pABC * (pABC - ab) * (pABC - ac) * (pABC - bc));
		double ADE = sqrt(pADE * (pADE - ad) * (pADE - ae) * (pADE - de));

		double areaTra = ABC - ADE;
        if (R < (ADE / areaTra)) {
			r = ad;
		} else {
			l = ad;
...
void solve() {
	double ab, ac, bc, r;
	scanD4(ab, ac, bc, r);
	cout << fixed << setprecision(10) << findAD(ab, ac, bc, r) << endl;
}
